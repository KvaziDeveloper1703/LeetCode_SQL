...
+ id is the primary key (a unique identifier for each employee).
...
Write a query to find all managers who have at least five direct reports (employees directly managed by them).
...
+ id — первичный ключ (уникальный идентификатор каждого сотрудника).
+ name — имя сотрудника.
...
Напишите SQL-запрос, который найдёт всех менеджеров, у которых есть как минимум 5 подчинённых (сотрудников, которых они непосредственно управляют).
...
FROM Employee
JOIN Employee AS Subordinate ON Employee.id = Subordinate.managerId
