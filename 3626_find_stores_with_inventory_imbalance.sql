...
Definitions:
    - For each store:
        - Find the most expensive product (maximum price) and its quantity;
        - Find the cheapest product (minimum price) and its quantity.
...
WITH inventory_with_ranks AS (
    SELECT inventory.store_id, inventory.product_name, inventory.quantity, inventory.price,
        ROW_NUMBER() OVER (PARTITION BY inventory.store_id ORDER BY inventory.price DESC) AS most_expensive_rank,
        ROW_NUMBER() OVER (PARTITION BY inventory.store_id ORDER BY inventory.price ASC) AS cheapest_rank,
        COUNT(*) OVER (PARTITION BY inventory.store_id) AS product_count
    FROM inventory
),
...
SELECT stores.store_id, stores.store_name, stores.location, store_inventory_summary.most_exp_product, store_inventory_summary.cheapest_product, ROUND(store_inventory_summary.cheapest_quantity / store_inventory_summary.most_expensive_quantity, 2) AS imbalance_ratio
